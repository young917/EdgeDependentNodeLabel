...
def _HypergraphPredictor__eval_i(args):
    ...
    if(len(res) != 0):
        ...
        pred = ((1-a)*ratio + (1-c) - np.sqrt((1-a)**2*ratio**2+(1-c)**2+2*(a*c+(a+c)-1)*ratio))/(2*ratio) # return probability of basket
